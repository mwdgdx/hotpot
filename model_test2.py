...
#         word_mat 是一个字典
    ...
    def forward(self, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs, context_lens, start_mapping, end_mapping, all_mapping, return_yp=False, is_support_word):
...
#       bsz： batch size
#       input_len context的每句话长度    context: b*c*d
#       memory_len question 每句话的长度 memory : b*q*d
...
#       b*c*n  = b*c*1 + b*1*q + b*c*q
        att = att - 1e30 * (1 - mask[:,None])
#       Biattention 用的是ques_mask: b*q
#       att 维度为： b*c*q
#       mask[:,None] 为： b*n*1？？ 难道不应该是b*1*n? mask[:,None] 是怎么运作的？
        weight_one = F.softmax(att, dim=-1)
#       b*c*q 关于n(question_lens)做softmax
